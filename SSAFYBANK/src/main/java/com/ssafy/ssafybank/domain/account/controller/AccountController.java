...
public class AccountController {
    ...
    @PostMapping("/create")
    public ResponseEntity<?> createAccount(@RequestBody @Valid AccountCreateRequestDto accountCreateRequestDto, @AuthenticationPrincipal LoginUser loginUser, BindingResult bindingResult){

        String memberUuid = loginUser.getMember().getMemberUuid(); // 강제로 준 값 로그인 구현 시 이 부분만 바뀜
        System.out.println("memner0 +"+ memberUuid);
